...
A topic and an announcer are different things and not two spellings of one. A topic is a single line that replaces the last one under a voice channel's name — a tally worth glancing at. An account is a message in a channel somebody scrolls back through, rewritten as the thing it describes grows — a summary worth reading later, and one of them per evening however many times that evening's room emptied. `scoreboard` uses the first and `summary` the second.

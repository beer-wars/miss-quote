...
`post_transcripts` puts the last `transcript_lines` of what the room has said into **one message** in the same `channel` the summary goes to, and rewrites that message as the room talks. `transcript_lines` is a maximum rather than a promise: a message holds 2,000 characters and a ring holds lines, so a room talking in paragraphs shows fewer of them, with the oldest dropped off the top first. There is no cap on a single line unless a channel sets `transcript_line_limit` — a long utterance is shown whole and costs the lines above it, since a sentence cut at a number tells you nothing about the words after it. The feed also leaves 100 characters of the message unspent, which nothing needs and which costs a line of transcript against a message Discord would refuse. Whole lines come off rather than characters — cutting the block partway would take the code fence off the front of it, and an unfenced feed loses the monospace and hands whatever the transcriber returned back to Markdown. It is off unless a channel asks for it, and that is a deliberate asymmetry rather than caution: the transcript on disk is a file in a volume with a retention window, while the same words in a text channel are permanent, searchable, and readable by everyone in the server rather than everyone who was in the room.
